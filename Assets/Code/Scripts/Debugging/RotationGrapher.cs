...
public class RotationGrapher : Graphic
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        var samples = collector.GetSamples(ResolveSeries());
    ...
    private Vector2 SampleToLocalPosition(ClientTelemetryCollector.Sample s, float minTime, Rect rect, float width, float height)
    {
        ...
        // Value [-180, 180] → Y in [0, height] (0 at bottom, 1 at top), centred
        float vNorm = (s.value + 180f) / 360f; // map [-180,180] to [0,1]
    ...
    private TelemetrySeries ResolveSeries()
    {
        switch (rotationAxis)
        {
            case RotationGraphAxis.Pitch:
                return TelemetrySeries.RotationPitch;
            case RotationGraphAxis.Yaw:
                return TelemetrySeries.RotationYaw;
            case RotationGraphAxis.Roll:
                return TelemetrySeries.RotationRoll;
            default:
                return TelemetrySeries.RotationPitch;
        }
    }
